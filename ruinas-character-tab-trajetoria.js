...
R.charTabs.trajetoria=(c,a)=>{
  return `<article class="panel"><h2>${a.length} capítulos</h2><div class="list">${a.map(n=>{
    ...
    return `<div class="row click" data-go="capitulo/${n}"><strong>Capítulo ${n} — ${E(ch.t)}</strong><p>${E(ch.s)}</p>${companions.length?`<div class="tags">${companions.map(name=>`<span class="tag">${E(name)}</span>`).join('')}</div>`:''}</div>`;
  }).join('')}</div></article>`;
};
})();